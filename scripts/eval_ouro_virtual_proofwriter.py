#!/usr/bin/env python3
"""Evaluate a virtual-depth Ouro lens against the matching logit lens."""
...
import torch
import transformers
from fit_ouro_virtual import OuroVirtualAdapter
...
def main() -> None:
    ...
    parser.add_argument("--only-layer", type=int, default=None)
    ...
    if args.only_layer is not None:
        if args.only_layer not in layers:
            raise SystemExit(f"requested layer {args.only_layer} is not in lens sources {layers[0]}..{layers[-1]}")
